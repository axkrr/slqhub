...
function CaptureToken() {
  try {
    ...
    if (!accessToken || !refreshToken) {
      $.notify('重庆大学饭卡', 'Token 捕获失败', 'oauth/token 响应中未找到 access_token 或 refresh_token')
      $.done()
      return
    }
    ...
    let root = {}
    try {
      root = JSON.parse($.read('#CQU') || '{}')
    } catch (e) {
      root = {}
    }

    root.CardBalance = root.CardBalance || {}
    root.CardBalance.Tokens = root.CardBalance.Tokens || {}
    root.CardBalance.Tokens.AccessToken = accessToken
    root.CardBalance.Tokens.RefreshToken = refreshToken
    root.CardBalance.Tokens.UpdatedAt = Date.now()
    root.CardBalance.Info = root.CardBalance.Info || {}
    root.CardBalance.Info.Name = name
    root.CardBalance.Info.Sno = sno
    $.write(JSON.stringify(root), '#CQU')

    $.notify(
      '重庆大学饭卡',
      'Token 写入成功',
      name + ' · 学号 ' + sno + '\naccess_token + refresh_token 已保存'
    )
  } catch (e) {
    $.notify('重庆大学饭卡', 'Token 解析失败', e.message || '未知错误')
  }
  $.done()
}
